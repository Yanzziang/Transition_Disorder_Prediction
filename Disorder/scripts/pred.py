...
def combine(xdir, sequence):
    ...
    #df.to_csv("/Users/yzza/Desktop/Protein_Transitions_Disorder_prediction/tmp.csv")
    return df
...
def prediction(xdir, df):
    ...
    model_file = xdir + "/../../models/Res-BiLstm.h5"
...
def main():
    ...
    label = ['D' if ele >= 0.5 else 'O' for ele in result]
    ...
    sequence_name = os.path.basename(sequence).split('.')[0] 
    outfile = xdir + '/../outcome/{0}.rb'.format(sequence_name)
    with open(outfile, 'w') as file:
        title = 'Disorder prediction by Res-BiLstm for {0}'.format(sequence) + '\n'
